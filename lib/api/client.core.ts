...
// Not exported by this tier's @/types barrel — each name below backs
// admin/pro/premium-only methods this tier's UI never calls. Local
// Record stand-ins keep the class identical to the full client instead
// of forking method bodies. See out/phases/phase-21-api-client-split.md.
type ImportHistoryForPage = Record<string, unknown>;
type SupportTicket = Record<string, unknown>;
type SupportTicketReply = Record<string, unknown>;
type SupportTicketOwnerProduct = Record<string, unknown>;
type CreateSupportTicketInput = Record<string, unknown>;
type CreateSupportTicketReplyInput = Record<string, unknown>;
type UpdateSupportTicketInput = Record<string, unknown>;
type ProductReview = Record<string, unknown>;
type CreateProductReviewInput = Record<string, unknown>;
type UpdateProductReviewInput = Record<string, unknown>;
type ReviewEligibilitySlot = Record<string, unknown>;
type ClientPortalStats = Record<string, unknown>;
type SupplierPortalStats = Record<string, unknown>;
type CreateCheckoutInput = Record<string, unknown>;
type CheckoutSessionResponse = Record<string, unknown>;
type GenerateLabelInput = Record<string, unknown>;
type GenerateLabelResponse = Record<string, unknown>;
type AddTrackingInput = Record<string, unknown>;
type GetRatesInput = Record<string, unknown>;
type GetRatesResponse = Record<string, unknown>;
type SystemConfig = Record<string, unknown>;
type UpdateSystemConfigInput = Record<string, unknown>;
