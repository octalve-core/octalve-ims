/**
 * Batch SSR review context for order detail — Core tier variant.
 * Reviews (lib/product-reviews) are premium-exclusive, so this tier's
 * order/invoice detail routes get no review data at all: every product is
 * reported as ineligible with no reviews, which naturally disables any
 * "write a review" affordance downstream (ProductReviewsSection itself is
 * premium and isn't rendered in this tier either — see
 * components/shared/ProductLineItemsList.core.tsx). Picked by
 * scripts/export-tier.ts in place of the default file when exporting Core
 * (see order-review-context-data.pro.ts for the identical Pro variant).
 * Every one of this file's 7 callers (order/invoice detail routes) and its
 * 3 downstream type-only consumers (OrderItemsCard.tsx, InvoiceItemsCard.tsx,
