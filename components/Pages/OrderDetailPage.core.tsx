/**
 * Order Detail Page — Core tier variant.
 * Same as OrderDetailPage.tsx but without the Stripe client-side
 * reconciliation fallback (hooks/use-stripe-checkout-return.ts is
 * pro-bucketed) and without carrier/shipping display
 * (components/shipping is premium-bucketed) — plain tracking-number text
 * only. Picked by scripts/export-tier.ts in place of the default file when
 * exporting Core. Keep in sync with OrderDetailPage.pro.tsx (which keeps
 * the Stripe hook) and the admin sibling AdminOrderDetailContent.{core,pro}.tsx.
 */
