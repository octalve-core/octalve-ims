/**
 * Order Detail Page — Pro tier variant.
 * Same as OrderDetailPage.tsx but without carrier/shipping display
 * (components/shipping is premium-bucketed, excluded from Pro too) — plain
 * tracking-number text only. Keeps the Stripe checkout-return client hook
 * (hooks/use-stripe-checkout-return.ts is pro-bucketed — Pro has it).
 * Picked by scripts/export-tier.ts in place of the default file when
 * exporting Pro. Keep in sync with OrderDetailPage.core.tsx (which also
 * drops the Stripe hook) and the admin sibling AdminOrderDetailContent.{core,pro}.tsx.
 */
    ...
    <AppShell>
      <PageContentWrapper>
        <div className={APP_SHELL_DETAIL_CLASS}>
          ...
          {/* Read-only badges (no premium carrier-tracking display in this tier) */}
          <OrderStatusBadges
            status={order?.status}
