...
/**
 * Pro tier variant — no shipping label auto-generation / carrier tracking
 * display (components/shipping is premium-bucketed, excluded from Pro too)
 * — manual tracking entry only. Keeps the Stripe checkout-return client
 * hook (hooks/use-stripe-checkout-return.ts is pro-bucketed — Pro has it).
 * Picked by scripts/export-tier.ts in place of the default file when
 * exporting Pro. Keep in sync with AdminOrderDetailContent.core.tsx (which
 * also drops the Stripe hook) and with the non-admin sibling
 * OrderDetailPage.{core,pro}.tsx.
 *
