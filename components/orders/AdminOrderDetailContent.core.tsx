...
/**
 * Core tier variant — no Stripe client-side reconciliation fallback
 * (hooks/use-stripe-checkout-return.ts is pro-bucketed) and no shipping
 * label auto-generation / carrier tracking display
 * (components/shipping is premium-bucketed) — manual tracking entry only.
 * Picked by scripts/export-tier.ts in place of the default file when
 * exporting Core. Keep in sync with AdminOrderDetailContent.pro.tsx (which
 * keeps the Stripe hook — Pro has lib/payments — but drops shipping the
 * same way) and with the non-admin sibling OrderDetailPage.{core,pro}.tsx.
 *
  ...
  return (
    <PageContentWrapper>
      <div className={APP_SHELL_DETAIL_CLASS}>
        ...
        {/* REQ-0208 read-only badges (no premium carrier-tracking display in this tier) */}
        <OrderStatusBadges
          status={order?.status}
