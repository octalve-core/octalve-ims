...
4. **"Keep me signed in" checkbox** (`components/Pages/LoginPage.tsx`, added post-port): the server-side `rememberMe` branch (`login/route.ts` → `REFRESH_DAYS.rememberMe`/`.short`) existed from Phase 3 but had no UI control — every login silently passed the context's old no-op default. Now `AuthContext.login(email, password, rememberMe)` forwards an explicit checkbox value (unchecked by default, 1-day refresh token); checking it opts into the 30-day refresh token. This is a real behavior change from the prior implicit default (the old `login()` omitted the field entirely, which the API defaults to `true`/30 days when absent) — unchecked is now the common case.
