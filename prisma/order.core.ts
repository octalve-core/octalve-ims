...
export async function cancelOrder(orderId: string, userId: string) {
  ...
  if (shouldRefund) {
    ...
    if (paymentIntentId) {
      // Core: no Stripe integration (@/lib/stripe is Pro-tier) -- paymentIntentId
      // is never actually set on a Core order, so this never fires in practice;
      // kept structurally identical to the default file minus the Stripe call.
      console.error(
        "Stripe refund needed during order cancellation but Stripe is not available in this tier",
      );
    }
  }
